...
class TestTrueSkillRanker:
    ...
    def test_pairwise_update_increases_winner_decreases_loser(self):
    ...
    def test_kway_conversion_ordering(self):
        ...
        # Get initial scores
        initial_scores = {
            crash_id: ranker.get_score(crash_id)
        ...
        for crash_id in ["a", "b", "c", "d"]:
            assert final_scores[crash_id] != initial_scores[crash_id], f"{crash_id} score should change"
    ...
    def test_weight_scaling_reduces_update_magnitude(self):
        ...
        ranker2.update_with_ordinal(result, weight=0.5)  # Lower weight (smaller changes)
    ...
    def test_unseen_crash_gets_default_rating(self):
    ...
    def test_snapshot_and_restore(self):
        ...
        for crash_id in ["a", "b", "c"]:
            assert ranker1.get_score(crash_id) == ranker2.get_score(crash_id), f"Scores should match for {crash_id}"
            assert ranker1.get_uncertainty(crash_id) == ranker2.get_uncertainty(crash_id), f"Uncertainties should match for {crash_id}"
    
    def test_get_score_returns_mu(self):
    ...
    def test_get_uncertainty_returns_sigma(self):
    ...
    def test_snapshot_contains_all_crashes(self):
        ...
        # Assert
        assert "x" in snapshot, "Snapshot should contain x"
        assert "y" in snapshot, "Snapshot should contain y"
        assert "z" in snapshot, "Snapshot should contain z"
        
        for crash_id in ["x", "y", "z"]:
            assert "mu" in snapshot[crash_id], f"Snapshot should contain mu for {crash_id}"
            assert "sigma" in snapshot[crash_id], f"Snapshot should contain sigma for {crash_id}"
