...
from crash_tournament.interfaces import Judge
from crash_tournament.judges.cursor_agent_judge import CursorAgentJudge
from crash_tournament.judges.cursor_agent_streaming_judge import (
...
from crash_tournament.logging_config import get_logger, setup_logging
...
def create_crash_from_path(filepath: str) -> Crash:
    ...
    return Crash(crash_id=crash_id, file_path=file_path)


def create_judge_instance(
    judge_type: str, timeout: float = 300.0, prompt_file: str | None = None
) -> Judge:
  ...
  uv run python crash_tournament/rank_crashes_demo.py --judge dummy crash1.json crash2.json
    ...
    try:
        ...
        if judge_type == "dummy":
            ...
            ground_truth: dict[str, float] = {
                crash.crash_id: float(i) for i, crash in enumerate(crashes)
            }
            judge = SimulatedJudge(ground_truth, noise=0.1)
        else:
            judge = create_judge_instance(
                judge_type, timeout=timeout, prompt_file=prompt
            )
        ...
        for rank, crash_id in enumerate(result.ordered_ids, 1):
            ...
            crash_obj: Crash = next(c for c in crashes if c.crash_id == crash_id)
            print(f"{rank}. {crash_id}")
            print(f"   File: {crash_obj.file_path}")
            print()
