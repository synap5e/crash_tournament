...
class RunConfig:
    ...
    snapshot_every: int = (
        10  # print progress every N matchups (snapshots saved on every update)
    )
    ...
    def __post_init__(self) -> None:
        ...
        if not (2 <= self.matchup_size <= 7):
            raise ConfigurationError(
                f"matchup_size must be between 2 and 7, got {self.matchup_size}"
            )
        if self.budget <= 0:
...
class Orchestrator:
    ...
    def __init__(
        ...
        ]()  # List of (matchup, exception_type, exception_msg)
    ...
    def run(self) -> dict[str, float]:
        ...
        with ThreadPoolExecutor(max_workers=self.config.max_workers) as executor:
            futures = dict[
                Future[OrdinalResult], tuple[Sequence[str], list[Crash]]
            ]()  # future -> (matchup_ids, crashes) mapping for error reporting
            ...
            while self.evaluated_matchups < self.config.budget:
                ...
                remaining_budget = (
                    self.config.budget - self.evaluated_matchups - in_flight
                )
                ...
                while len(futures) < self.config.max_workers and remaining_budget > 0:
                    ...
                    available_crashes = [
                        c for c in crash_ids if c not in self.in_flight_crashes
                    ]
                    ...
                    matchup_ids = self.selector.select_matchup(
                    ...
                    crashes = [
                        self.fetcher.get_crash(crash_id) for crash_id in matchup_ids
                    ]
    ...
    def _process_completed_futures(
        self,
        futures: dict[Future[OrdinalResult], tuple[Sequence[str], list[Crash]]],
        wait_all: bool = False,
    ) -> None:
        """
        ...
        for future in done_futures:
            ...
            try:
                ...
                if self.config.matchup_size <= 1:
                    raise ConfigurationError(
                        f"matchup_size must be >= 2, got {self.config.matchup_size}"
                    )
                ...
                if (
                    self.evaluated_matchups % MILESTONE_INTERVAL == 0
                    and self.evaluated_matchups > self.last_milestone
                ):
                ...
                if (
                    self.total_evaluations >= EARLY_ABORT_THRESHOLD
                    and self.failed_evaluations == self.total_evaluations
                ):
                    raise RuntimeError(
                        f"100% failure rate in first {EARLY_ABORT_THRESHOLD} evaluations - aborting"
                    )
                ...
                if self.total_evaluations >= LATE_ABORT_THRESHOLD:
                    ...
                    if failure_rate > FAILURE_RATE_LIMIT:
                        raise RuntimeError(
                            f"Failure rate {failure_rate:.1%} exceeds {FAILURE_RATE_LIMIT:.0%} threshold - aborting"
                        )
    ...
    def _print_progress(self) -> None:
        ...
        print(
            f"Progress: {self.evaluated_matchups}/{self.config.budget} matchups ({progress:.1f}%), {len(self.in_flight_crashes)} crashes in-flight"
        )
